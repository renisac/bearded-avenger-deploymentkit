...
until curl --silent --fail "http://${CIF_STORE_NODES}/_cluster/health?wait_for_status=yellow&timeout=180s"
do
...
if [[ $status_code == 200 ]]
then
  until curl --silent --fail "http://${CIF_STORE_NODES}/tokens/_search"
  do
  ...
  if [ -z ${ADMIN_TOKEN_GROUPS} ]
  then
    ADMIN_TOKEN_GROUPS="everyone"
  fi
  echo ""
  ...
  /cif_venv/bin/cif-store \
    -d \
    --store elasticsearch \
    --nodes ${CIF_STORE_NODES} \
    --token-create-admin \
    --token ${CIF_TOKEN} \
    --token-groups ${ADMIN_TOKEN_GROUPS}
  ...
  if [ -z ${HUNTER_TOKEN_GROUPS} ]
  then
    HUNTER_TOKEN_GROUPS="everyone"
  fi
  echo ""
  ...
  /cif_venv/bin/cif-store \
    ...
    --nodes ${CIF_STORE_NODES} \
    --token-create-hunter \
    --token ${CIF_HUNTER_TOKEN} \
    --token-groups ${HUNTER_TOKEN_GROUPS}
  ...
  if [ -z ${CSIRTG_SMRT_TOKEN_GROUPS} ]
  then
    CSIRTG_SMRT_TOKEN_GROUPS="everyone"
  fi
  echo ""
...
#  chmod 0640 /etc/cif/csirtg-smrt.yml
  /cif_venv/bin/cif-store \
    ...
    --nodes ${CIF_STORE_NODES} \
    --token-create-smrt \
    --token ${CSIRTG_SMRT_TOKEN} \
    --token-groups ${CSIRTG_SMRT_TOKEN_GROUPS}
  echo ""
  ...
  if [ -z ${HTTPD_TOKEN_GROUPS} ]
  then
    HTTPD_TOKEN_GROUPS="everyone"
  fi
  echo ""
  ...
  /cif_venv/bin/cif-store \
    ...
    --nodes ${CIF_STORE_NODES} \
    --token-create-httpd \
    --token ${CIF_HTTPD_TOKEN} \
    --token-groups ${HTTPD_TOKEN_GROUPS}
fi
